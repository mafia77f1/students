...
import { Clock, Zap, Trophy, Play, Target, CheckCircle2, Star, BarChart3 } from "lucide-react";
        ...
        <Card className="gradient-primary text-primary-foreground glow-primary overflow-hidden">
          <CardContent className="pt-5 pb-5">
            <div className="flex items-center justify-between mb-3">
              <div>
                <h1 className="text-xl font-bold">مرحباً، {profile.name || (isTeacher ? "أستاذ" : "طالب")} 👋</h1>
                <p className="text-sm opacity-80 mt-0.5">
                  {isTeacher ? "أستاذ" : `${rank.icon} ${rank.label} • المستوى ${profile.level}`}
                </p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-3 gap-2">
        {isTeacher ? (
          [
            ...
            { label: "المستوى", value: profile.level, icon: BarChart3 },
          ].map((stat, i) => (
